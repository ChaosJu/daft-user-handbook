...
赶时间就只看 **[配置决策](00-cheatsheet.md)**——按「禁区 → 读侧 → partition → actor → 批大小 → 写出」走一遍，每步给公式和算例，覆盖日常九成的问题，机制推导仍留在下面各页。

| 页 | 回答什么 |
|---|---|
| [0. 配置决策](00-cheatsheet.md) | 这个作业该怎么配：driver 出口禁区、scan 切分、partition 怎么定、actor 资源与并行度怎么算、批大小、写出、症状对照 |
